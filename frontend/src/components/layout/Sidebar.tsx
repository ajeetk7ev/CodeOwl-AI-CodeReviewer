...
export function Sidebar({ className }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={cn("hidden md:flex flex-col h-full bg-[#0C0C0C] border-r border-[#1F1F1F] relative overflow-hidden", className)}>
        {/* Ambient glow effect */}
      ...
      {/* Mobile Bottom Bar */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#0C0C0C]/80 backdrop-blur-sm border-t border-[#1F1F1F] rounded-t-2xl z-50 flex items-center justify-around px-2">
        {allMenuItems.map((route) => (
          <Link
            key={route.href}
            to={route.href}
            className="relative flex flex-col items-center justify-center gap-1 p-2 rounded-lg"
          >
            {route.active && (
                <motion.div
        ...
        <Button 
            variant="ghost" 
            className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg text-muted-foreground hover:text-red-500 hover:bg-red-500/10"
            onClick={() => logout()}
        >
